...
def test_conditional_transition_is_idempotent():
    table = Mock()
    assert transition(table, "job-1", "QUEUED", "PROCESSING") is True
    table.update_item.side_effect = error("ConditionalCheckFailedException")
    assert transition(table, "job-1", "QUEUED", "PROCESSING") is False


def test_ingest_valid_request(monkeypatch):
    mod = load("ingest.app")
    mod.table = Mock()
    mod.s3 = Mock()
    mod.sqs = Mock()
    mod.BUCKET = "bucket"
    mod.QUEUE_URL = "queue"
    result = mod.handler({"body": json.dumps({"fileName": "video.mp4", "contentType": "video/mp4"})}, SimpleNamespace(aws_request_id="req"))
    assert result["statusCode"] == 202
    body = json.loads(result["body"])
    assert body["status"] == "QUEUED"
    mod.table.put_item.assert_called_once()
    mod.s3.put_object.assert_called_once()
    mod.sqs.send_message.assert_called_once()


@pytest.mark.parametrize("body", [{}, {"fileName": "bad/name.mp4", "contentType": "video/mp4"}, {"fileName": "x.mp4"}])
def test_ingest_invalid_request(monkeypatch, body):
    mod = load("ingest.app")
    result = mod.handler({"body": json.dumps(body)}, None)
